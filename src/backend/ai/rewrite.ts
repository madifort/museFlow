...
export interface RewriteOptions extends PromptOptions {
  ...
  tone?: 'formal' | 'casual' | 'professional' | 'creative' | 'academic' | 'conversational';
...
export async function handleRewrite(
  ...
  try {
    ...
    // Check cache first
    const cacheKey = `rewrite:${JSON.stringify(options)}:${text.substring(0, 100)}`;
    const cachedResponse = await getCachedResponse(cacheKey, 'rewrite');
    ...
    // Cache the response
    await saveToCache('rewrite', cacheKey, aiResponse);
...
function buildRewritePrompt(text: string, options: RewriteOptions): string {
  ...
  const improvementsInstruction = options.improvements && options.improvements.length > 0
...
function getStyleInstruction(style?: RewriteOptions['style']): string {
  ...
  if (style.simplify) {
    instructions.push('Simplify complex sentences and use clear, straightforward language.');
  }
  
  if (style.descriptive) {
    instructions.push('Add descriptive language and vivid details where appropriate.');
  }
...
function parseRewriteResponse(response: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line && !line.toLowerCase().includes('rewritten') && !line.toLowerCase().includes('version')) {
      startIndex = i;
      break;
    }
  }
 ...
 */
function calculateChanges(originalText: string, rewrittenText: string): RewriteResult['changes'] {
...
export async function handleBatchRewrite(
  ...
  logger.info('Batch rewriting completed', { 
    successful: results.filter(r => !r.rewrittenText.startsWith('Error:')).length,
