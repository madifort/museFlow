...
export async function handleSummarize(
  ...
  try {
    ...
    // Check cache first
    const cacheKey = `summarize:${JSON.stringify(options)}:${text.substring(0, 100)}`;
    const cachedResponse = await getCachedResponse(cacheKey, 'summarize');
    ...
    // Cache the response
    await saveToCache('summarize', cacheKey, aiResponse);
...
function buildSummarizePrompt(text: string, options: SummarizeOptions): string {
  const lengthInstruction = getLengthInstruction(options.summaryLength || 'medium');
 ...
 */
function parseSummarizeResponse(response: string, options: SummarizeOptions): Omit<SummarizeResult, 'metadata'> {
...
function calculateConfidence(response: string): number {
  ...
  if (response.includes('The') || response.includes('This') || response.includes('It')) {
    confidence += 0.1;
  }
