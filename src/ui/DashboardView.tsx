          ...
          {quickActions.map((action) => (
            ...
            >
              <div className="flex flex-col items-center text-center">
                <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${action.gradient} flex items-center justify-center text-white text-sm font-bold shadow-lg mb-2 group-hover:scale-110 transition-transform duration-200`}>
                  {action.icon}
                </div>
                <h4 className="font-semibold text-gray-900 dark:text-white text-xs mb-1">{action.title}</h4>
        ...
        <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-lg shadow-lg border border-white/20 dark:border-gray-700/20 p-4 transition-colors duration-300">
          <div className="space-y-3">
            {recentActivity.map((activity, index) => (
              ...
              >
                <div className={`w-6 h-6 rounded-lg bg-${activity.color}-100 dark:bg-${activity.color}-900/20 flex items-center justify-center`}>
                  <span className="text-xs">{activity.icon}</span>
                </div>
                <div className="flex-1">
                  <p className="text-xs font-medium text-gray-900 dark:text-white">{activity.text}</p>
